...
namespace MultiScreenshotCaptureNamespace
{
	...
	public class MultiScreenshotCapture : EditorWindow
	{
		...
		[MenuItem("Funzilla/Multi Screenshot Capture")]
		...
		private void Awake()
		{
			...
			MutilScreenHotKey.OnKeyDown += CaptureButton;
		}
		...
		private void OnGUI()
		{
			...
			CaptureKey = (KeyCode)EditorGUILayout.EnumPopup("Capture Hotkey", CaptureKey);
			if (Application.isPlaying)
			{
				if (MutilScreenHotKey.Instance == null)
				{
					MutilScreenHotKey.Instance = new GameObject("mutilScreenHotKey").AddComponent<MutilScreenHotKey>();
				}
				if (MutilScreenHotKey.Instance)
				{
					MutilScreenHotKey.Instance.SetKey(CaptureKey);
				}
			}
		...
		private MultiScreenshotCapture instance;
		...
		public void CaptureButton()
		{
			...
			Debug.Log("resolution number: " + resolutions.Count);
